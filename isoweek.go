...
// startOrdinal returns the ordinal day number of Monday of the
// given week. Return value may be negative or bigger than number of
// days in a year.
func startOrdinal(year, week int) (ordinal int) {
	return week*7 - isoWeekday(year, 1, 4) - 2
}
...
func StartTime(wyear, week int, loc *time.Location) (start time.Time) {
	return time.Date(wyear, 1, 1, 0, 0, 0, 0, loc).AddDate(0, 0, startOrdinal(wyear, week)-1)
}
